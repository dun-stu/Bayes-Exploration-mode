...
- Currently all probability-mode LaTeX uses fixed D/T notation across all scenarios. More intuitive per-scenario symbols could improve comprehension (e.g., S/F for spam, D/T for medical scenarios).
- Requires: new symbol fields in `ScenarioDefinition`, updating all LaTeX template strings in `computeRegionB`, Bayes formula panel, and careful symbol selection per scenario.
- Cross-cutting change touching template system (Layer 1), tree rendering (Layer 2), formula panel (Layer 5).
- **Status:** Not started — scoped as a separate enhancement subtask
- **Verify:** Each scenario uses appropriate notation in all probability-mode labels, formulas, and tooltips
