...
**2026-04-01 — Post-Layer 5 harmonisation.** Reconciled accumulated spec divergences and building-phase decisions from all Layer 5 subtasks (5.1–5.5) with the Implementation Details doc. Changes made:

1. **Part 3 current status:** Updated from "Layers 3 and 4 built, remaining: responsive layout and polish" to "Layers 3–5 built and verified — exploration mode complete. Next: Part 4."
2. **Building-phase decisions — all 13 items resolved:** Updated remaining deferred items: responsive breakpoint (768px, stacking), Bayes' rule formula toggle (three-zone format, text-style link, below-vis panel), first-time-user affordance (evaluated and not needed), responsive format selector (tighter padding at narrow breakpoint), hover tooltips (SVG `<title>` on compound labels, KaTeX `\htmlClass` tooltips on formula values, glossary cut).
3. **Tree node domain labels — leaf labels added:** Updated the Layer 3 implementation note to include Layer 5.4d leaf labels (test-outcome vocabulary below each leaf node, using `ByTestResultLabels`). Documents the reversal from 5.4c's exclusion decision.
4. **Tree first-level node colour distinction:** Added implementation note documenting blended midpoint shades (#ED8A30 condition-positive, #3B80AC condition-negative) that distinguish parent from leaf nodes. Change confined to `TREE_NODE_COLORS`.
5. **Branch label positioning:** Added implementation note for perpendicular offset (5.4d) and KaTeX branch label alignment with `textAlign` prop (5.4e).
6. **Combination bracket rendering:** Added implementation note for `\tfrac` stacked fractions in probability mode and spacing adjustments.
7. **`prefers-reduced-motion` support:** Added implementation note documenting the `usePrefersReducedMotion` hook and its effect on all three animation paths.
8. **Accessibility implementation:** Added comprehensive implementation note documenting ARIA attributes on both SVGs, keyboard navigation, `aria-live` announcer, and spatial-accessibility limitation.
9. **Cross-part flags resolved:** Updated sticky slider UX (verified Y2 handles it), N-change notification (transient slide-down, 4s dismiss), degenerate state displays (contextual notes with warm styling, probability-mode string fix), glossary (cut — tooltips cover the function).
10. **Scenario-adapted notation:** Added `conditionSymbol`/`testSymbol` fields to scenario data structure definition, `NotationSymbols` reference, and notation symbol coverage to library summary.
11. **Factory vocabulary:** Updated `test_positive_name` from "are rejected" to "are flagged" and `test_positive_name_singular` from "is rejected" to "is flagged" throughout (vocabulary tables, template examples, glossary examples, data structure examples, vocabulary repetition resolution note).
12. **Probability-mode rendering:** Added implementation note for `isLatex()` pattern expansion, `KaTeXInline` trust mode, and question-mark thin-space fix.
13. **Report-relevant notes:** Added three new Notes for Later entries: three-zone Bayes formula as vocabulary bridging architecture, accessibility as principled implementation with honest limitations, scenario-adapted notation as domain-appropriate formalism.

Also updated `Our Plan + Status.md`:
- Updated Part 3 status row from "Complete (Layers 3–4 built)" to "Complete (Layers 0–5 built)" with full Layer 5 summary.
- Updated next steps to reflect exploration mode complete, Part 4 next.
- Updated summary line to reflect all layers complete and harmonisation done.
